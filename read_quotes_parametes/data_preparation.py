...
from read_quotes_parametes.get_item import get_all_tables_from_data
...
def read_parameters(catalog: Catalog = None):
    ...
    for table in tables:
        ...
        if catalog_table:
            catalog_table.attributes = table[2] if table[2] is not None else ""
            catalog_table.parameters = table[3] if table[3] is not None else ""
            count += 1
        else:
